$(document).ready(function () {
    ...
    $("#allBtn").click(fetchTimesheetData);
    ...
    function displayPage(page) {
        ...
        $.each(paginatedItems, function (index, item) {
            ...
            let statusBadge = '<span class="badge bg-warning d-inline-flex align-items-center"><i class="ti ti-point-filled me-1"></i>' + item.status + '</span>';
            if (item.status === "Approved") {
                statusBadge = '<span class="badge bg-success d-inline-flex align-items-center"><i class="ti ti-point-filled me-1"></i>' + item.status + '</span>';
            } else if (item.status === "Rejected") {
                statusBadge = '<span class="badge bg-danger d-inline-flex align-items-center"><i class="ti ti-point-filled me-1"></i>' + item.status + '</span>';
            }
    ...
    function populateTable(data) {
        ...
        $.each(data, function (index, item) {
            let profilePicture = item.user && item.user.profilePicture
                ? `/Content/uploads/${item.user.profilePicture.split('/').pop()}` // Ensure correct URL
                : "/assets/img/users/default.jpg";
            let fullName = item.user ? (item.user.firstName || "") + " " + (item.user.lastName || "") : "N/A";
            let statusBadge = '<span class="badge bg-warning d-inline-flex align-items-center"><i class="ti ti-point-filled me-1"></i>' + item.status +'</span>';
            if (item.status === "Approved") {
